...
if __name__ == "__main__":
    ...
    # descriptor ECFP_SYBYL
    data_ECFP_SYBYL = data.reindex(columns = data.columns.tolist() + atom_type + [str(i) for i in range(nbits)])
    data_ECFP_SYBYL = data_ECFP_SYBYL.apply(ECFP_SYBYL, axis=1).fillna(0)
    data_ECFP_SYBYL.to_csv("ECFP_SYBYL.csv",index=False)
    os.remove("./molecule.mol2")